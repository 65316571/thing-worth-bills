...
export default function WishBoardPanel({ mobile = false }) {
  ...
  const filteredWishes = useMemo(() => {
    ...
    return wishes.filter((wish) => (wish.category || "未分类") === filter);
  }, [filter, wishes]);
  ...
  const handleAdd = async () => {
    ...
    try {
      await addWish({ ...form, targetPrice: parseFloat(form.targetPrice), status: "planning" });
      setForm({ name: "", targetPrice: "", category: "", priority: "medium", purpose: "", note: "" });
      setShowForm(false);
    } catch (requestError) {
    ...
    <section className={mobile ? "wish-mobile-shell" : "desktop-panel"}>
      ...
      <div className="wish-filter-row">
        ...
        {CATEGORIES.map((category) => (
          <button
            key={category}
            className={`wish-filter-chip ${filter === category ? "active" : ""}`}
      ...
      {showForm && (
        <div className="desktop-form-card wish-form-card">
          <div className="desktop-form-grid two-col">
            <input className="form-input" placeholder="物品名称 *" value={form.name} onChange={(e) => setForm((prev) => ({ ...prev, name: e.target.value }))} />
            <input className="form-input" type="number" placeholder="目标价格 *" value={form.targetPrice} onChange={(e) => setForm((prev) => ({ ...prev, targetPrice: e.target.value }))} />
          </div>
          <div className="desktop-form-grid two-col">
          ...
          <input className="form-input" placeholder="购买用途 / 使用场景" value={form.purpose} onChange={(e) => setForm((prev) => ({ ...prev, purpose: e.target.value }))} />
          <textarea className="form-textarea" placeholder="补充说明（例如理想版本、想等的活动）" value={form.note} onChange={(e) => setForm((prev) => ({ ...prev, note: e.target.value }))} />
        ...
        {filteredWishes.map((wish) => (
          <article className="wish-board-card" key={wish.id}>
            ...
            <div className="wish-board-meta-row">
              ...
              <span className="wish-board-chip purpose">{wish.priority === "high" ? "高优先级" : wish.priority === "low" ? "低优先级" : "短期目标"}</span>
            </div>
